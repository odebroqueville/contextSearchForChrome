...
const ICON32 = '32px'; // icon width is 32px plus 3px margin/padding
...
function init() {
	...
	chrome.storage.sync.get(null, (data) => {
		...
		chrome.storage.local.get(null, (store) => {
			let favicons = store.favicons;
			if (logToConsole) console.log(favicons);
			for (let id in favicons) {
				searchEngines[id].base64 = favicons[id];
			}
			if (logToConsole) console.log(searchEngines);
		});
	});
...
function handleRightClickWithoutGrid() {
	let selectedText = getSelectedText();
	sendSelectionToBackgroundScript(selectedText);
}
