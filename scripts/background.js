...
const notifyEnableStorageSync = chrome.i18n.getMessage('notifyEnableStorageSync');
const notifySearchEnginesLoaded = chrome.i18n.getMessage('notifySearchEnginesLoaded');
const titleMultipleSearchEngines = chrome.i18n.getMessage('titleMultipleSearchEngines');
...
const notifySearchEngineWithKeyword = chrome.i18n.getMessage('notifySearchEngineWithKeyword');
...
let contextsearch_displayFavicons = false; // Chrome doesn't support this feature yet (Nov 2019)
...
function saveSearchEnginesToStorageSync(blnNotify, blnUpdateContentScripts) {
	return new Promise((resolve, reject) => {
		...
		for (let id in searchEnginesLocal) {
			store.favicons[id] = searchEnginesLocal[id].base64;
			delete searchEnginesLocal[id].base64;
		}
		if (contextsearch_cacheFavicons) {
			if (logToConsole) console.log('cacheFavicons is enabled, saving favicons to local storage..\n');
